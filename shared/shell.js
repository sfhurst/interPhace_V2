...
window.InterPhaceShell = (() => {
  ...
  const runtime = Object.freeze({
    ...
    reportDiagnostic(phace, entry) {
      if (typeof phace !== "string" || !entry || typeof entry !== "object") return false;
      return postRuntimeMessage("interPhace:runtime-diagnostic", { phace, entry: cloneRuntimeState(entry) });
    },
  ...
  function bind(config) {
    ...
    if (appRoot.dataset.runtimeDiagnosticClicksBound !== "1" && !new URLSearchParams(window.location.search).has("persistent-engine")) {
      appRoot.dataset.runtimeDiagnosticClicksBound = "1";
      appRoot.addEventListener("click", event => {
        const target = event.target instanceof Element
          ? event.target.closest("button, input, select, [data-row], [data-col], [data-name]")
          : null;
        runtime.reportDiagnostic(name, {
          stage: "visible-click",
          pageUrl: `${location.pathname}${location.search}`,
          targetId: target?.id || null,
          targetName: target?.dataset?.name || null,
          row: target?.dataset?.row ?? null,
          col: target?.dataset?.col ?? null,
    ...
    function syncPlaying() {
      ...
      const runtimeAccent = typeof getRuntimeAccent === "function" ? getRuntimeAccent(session) : null;
      auditionBtn.style.setProperty("--shell-accent", runtimeAccent || session?.accent || source?.color || accent || "#8d939c");
