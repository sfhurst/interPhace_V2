document.addEventListener("DOMContentLoaded", () => {
    ...
    const synthPromise = includeSynth
      ? waitForFrameAPI(
          ...
          win => win?.SynthPhaceAuditionEngine?.renderGlobalTrigger &&
            win?.SynthPhaceAuditionEngine?.renderArpNote
  ...
  function buildUtilityGrid(container, mode) {
    ...
    if (mode === "import") {
      ...
      projectHeader?.addEventListener("click", () => {
        const allExportButtons = [projectJsonButton, ...patchButtons, ...midiButtons, ...audioButtons].filter(Boolean);
  ...
  async function exportCurrentProject(container, { onAudioProgress = null } = {}) {
    ...
    if (!selectedProjectJson && selectedKeys.length === 1 && selectedMid === 0 && selectedWav === 0) {
