(() => {
  ...
  const FINAL_FADE_SECONDS = 0.020;
  ...
  function finalizeBuffer(buffer) {
    ...
    const paddingFrames = Math.max(fadeFrames, Math.round(0.030 * buffer.sampleRate));
    const meaningful = lastMeaningfulFrame(buffer);
    const endFrame = Math.min(buffer.length, meaningful + paddingFrames + 1);
    ...
    for (let channel = 0; channel < buffer.numberOfChannels; channel += 1) {
      ...
      const fadeStart = Math.max(0, outputLength - fadeFrames);
  ...
  async function renderCompleteAudition(legacyPatch, generation, auditionSettings) {
    ...
    while (true) {
      ...
      if (peakInTail(rendered) <= SILENCE_THRESHOLD) break;

      extraTail += EXTENSION_SECONDS;
      if (extraTail > MAX_EXTRA_TAIL_SECONDS) {
        throw new Error("synthPhace audition did not decay to silence within the offline render safety limit.");
      }
    }
