...
FMEngine.build = function (ctx, baseFreq, fmParams, noteLength) {
  ...
  // Clean post-FM harmonic color. These oscillators are mixed beside the FM carrier,
  // so the FM modulators never act on the added 2nd/3rd harmonics.
  ...
  function addCleanHarmonics(parentFreq, parentGain) {
    ...
    [
      { multiple: 2, amount: 0.25 },
      { multiple: 3, amount: 0.08 },
    ].forEach(({ multiple, amount }) => {
      const oscillator = ctx.createOscillator();
      oscillator.type = "sine";
      oscillator.frequency.setValueAtTime(parentFreq * multiple, t0);

      const gainNode = ctx.createGain();
      gainNode.gain.value = parentGain * amount * harmonicColor;
