...
## Current position — V3 Build 553 planning checkpoint

The shell is established and functional. The first direct interPhace load in a browser tab session begins on the non-button startup splash while preserving the normal shared top and bottom rows; reloads restore the current persisted interPhace page, and a newly opened tab begins on the splash again. Each child Phace has eight silent local-only snapshots: B6 hold saves its complete local state, tap restores, and snapshot hold clears and compacts the slots. interPhace also has eight local-only full-project snapshots on B3 Import; B6 hold saves the complete project, tap restores and reloads, and snapshot hold clears and compacts the slots. Global project context, mixer, import/export, child settings, Phace navigation, and render-first global audition exist. V3 Build 553 establishes the forward direction: persistent live audition through one project-level audio host, with Phace-originating auditions continuing through UI navigation until explicit Stop. Existing V2 playback remains unchanged until it is intentionally migrated; export remains offline and deterministic.

V3 Build 554 adds the persistent static-host runtime shell. Root `index.html` now retains the runtime host and presents the existing iP/sP/aP/dP/nP/dnP pages in one visually unchanged full-screen view. This build establishes retained navigation and the host-to-Phace route bridge only; existing V2 audio remains page-owned and therefore unchanged until a live engine is migrated.

## Next major work

## Build 644 — UI control cleanup

- iP Project Length and the unused iP/sP/dP/aP/nP/dnP Timing controls are removed.
- Blank-sequencer iP fallback playback derives its span from the active visible drum-grid columns, matching retained live playback and construction export. Sequencer content remains its own span authority.
- Older saved projects and imported `project.json` files may retain retired fields; iP removes them without changing musical state when it next saves/imports.
